 ...
 * @author admin
 */
@WebServlet(name = "OrderConfirm", urlPatterns = {"/staff/OrderConfirm"})
public class OrderConfirm extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            SaleOrder findingSaleOrder = saleOrderDAO.getById(oid);
            if (findingSaleOrder.getSaleOrderId() == 0) {
                response.sendRedirect("OrderList");
                return;
            } else {
                findingSaleOrder.setProcessing(CalculatorService.parseLong(process));
                saleOrderDAO.updateById(oid, findingSaleOrder);
                response.sendRedirect("OrderList");
                return;
            }
        } catch (Exception e) {
