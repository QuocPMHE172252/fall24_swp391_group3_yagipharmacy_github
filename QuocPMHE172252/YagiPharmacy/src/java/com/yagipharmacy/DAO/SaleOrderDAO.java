...
public class SaleOrderDAO implements RowMapper<SaleOrder> {
    ...
    public Double getDailyRevenue() throws SQLException, ClassNotFoundException {
        String sql = """
                     SELECT SUM(total_price) AS total_value FROM sale_order WHERE (?)-cast(created_date as real)<86400000  AND is_deleted = 0
                     """;
        Double price = 0d;
        try (Connection con = SQLServerConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql);) {
            ps.setObject(1, new Date().getTime());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                price = rs.getDouble("total_value");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Double getAllRevenue() throws SQLException, ClassNotFoundException {
        String sql = """
                     SELECT SUM(total_price) AS total_value FROM sale_order where is_deleted = 0
                     """;
        Double price = 0d;
        try (Connection con = SQLServerConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql);) {
            ResultSet rs = ps.executeQuery();
